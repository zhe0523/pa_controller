...
# Static Idle 实际输出图图像池配置。地址和 size 来自 uio2 sysfs，这里只保留帧对齐策略。
...
config:
	...
	@echo "FPGA_UIO = image=$(FPGA_IMAGE_UIO_DEVICE) offset=$(FPGA_OFFSET_UIO_DEVICE) gain=$(FPGA_GAIN_UIO_DEVICE) frame_align=$(DDR_IMAGE_FRAME_ALIGN)"
	@echo "CORR = pkg=$(CORR_DEFAULT_PKG_NUM) image=$(CORR_DEFAULT_COL_NUM)x$(CORR_DEFAULT_ROW_NUM) offset_en=$(CORR_DEFAULT_OFFSET_EN) offset_adder=$(CORR_DEFAULT_OFFSET_ADDER_VALUE) gain_en=$(CORR_DEFAULT_GAIN_EN) gain_clip=$(CORR_DEFAULT_GAIN_CLIPPING_VALUE) defect_en=$(CORR_DEFAULT_DEFECT_EN)"
