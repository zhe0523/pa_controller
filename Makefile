...
# 配置寄存器写后立即读回校验次数。
# 0 表示只写不读；1 表示写后读一次；2 表示失败后重写并再读一次，以此类推。
# STR/STOP 等触发型寄存器始终只写不校验，避免读自清或脉冲型信号带来副作用。
PA_PU_WRITE_VERIFY_ATTEMPTS ?= 2
...
CFLAGS += -DPA_PU_WRITE_VERIFY_ATTEMPTS=$(PA_PU_WRITE_VERIFY_ATTEMPTS)
...
config:
	...
	@echo "PA_PU_WRITE = verify_attempts=$(PA_PU_WRITE_VERIFY_ATTEMPTS)"
