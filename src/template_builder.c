...
static int load_active_template(const char* path, uint16_t* dst, unsigned repeat_count) {
  ...
  for (unsigned repeat = 0; repeat < repeat_count; ++repeat) {
    uint16_t* repeat_dst = dst + (size_t)repeat * DEVICE_WIDTH * DEVICE_HEIGHT;
    for (unsigned row_index = 0; row_index < IMAGE_HEIGHT; ++row_index) {
      ...
      copy_active_row_to_device(repeat_dst, row_index, row);
    }
  }
...
int template_load_files(fpga_mem_t* mem) {
  ...
  int ret = 0;
  if (load_active_template(TEMPLATE_OFFSET_FILE, (uint16_t*)mem->offset_template, 1) != 0) {
    ret = -1;
  }
...
int template_make_gain(fpga_mem_t* mem) {
  ...
  for (unsigned repeat = 0; repeat < 2; ++repeat) {
    ...
    for (unsigned row = 0; row < IMAGE_HEIGHT; ++row) {
      ...
      copy_active_row_to_device(repeat_gain, row, row_buf);
